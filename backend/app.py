...
def index():
    return jsonify({
        "status": "server_ready",
        "message": "SkinCheckAI Backend API",
        "version": "1.1.0",
        "model_status": "(4-Class Classification)"
    })
...
def predict():
    if not model:
        return jsonify({'error':'Model AI dont detect anything'}), 500
    if 'file' not in request.files:
        return jsonify({'error':'No image file are uploaded'}), 400
    ...
    try:
        ...
        return jsonify({'error': 'An error happening while processing the image'}), 500
